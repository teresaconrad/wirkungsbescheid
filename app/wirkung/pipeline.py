...
def kategorien(gesamtbetrag: Decimal) -> list[Kategorie]:
    ...
    for k in _DATEN["kategorien_gesamtstaat"]:
        ...
        # Kalendertage-Anker für Kategorien ohne verifizierten Kostensatz:
        # Anteil × 365 — reine Arithmetik auf den Eurostat-Anteilen (P9-konform),
        # statt eines nichtssagenden Platzhaltersatzes.
        if not kat.einheiten and _DATEN.get("kalender_anker", {}).get("aktiv"):
            tage = (kat.anteil * D("365")).quantize(D("1"), rounding=ROUND_HALF_UP)
            jahr = _DATEN["deltas"]["jahr_bis"]
            kat.einheiten.append({
                "einheit": f"{tage} Tagen Ihres Steuerjahres",
                "anzahl": None, "bezugsjahr": jahr,
                "label": f"so viele der 365 Tage arbeitet Ihr Steuer-Euro {jahr} für diesen Bereich",
                "quelle": kat.quelle, "basis_zusatz": None,
            })
        result.append(kat)
    return result
...
def methodik_link() -> str | None:
    """URL zur Methodik-Dokumentation (S5.5). None → Satz endet ohne Link."""
    return _DATEN.get("methodik_link") or None
