...
def _normalisiere(img: Image.Image) -> bytes:
    # EXIF-Orientierung anwenden: Handyfotos tragen die Drehung nur als Metadatum —
    # ohne diesen Schritt geht ein Hochkant-Foto um 90° gedreht in die Extraktion.
...
def vorverarbeite_dateien(pfade: list[Path]) -> Vorverarbeitet:
    """Mehrere Dateien (z. B. ein Foto je Bescheidseite) zu EINEM Extraktions-
    Eingang zusammenführen. Reihenfolge = Reihenfolge der Auswahl beim Upload."""
    bilder: list[bytes] = []
    textteile: list[str] = []
    for pfad in pfade:
        vv = vorverarbeite(pfad)
        bilder.extend(vv.bilder)
        if vv.textlayer:
            textteile.append(vv.textlayer)
    return Vorverarbeitet(bilder=bilder,
                          textlayer="\n\n".join(textteile) or None)
