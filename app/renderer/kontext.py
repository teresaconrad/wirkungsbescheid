...
from ..wirkung import ebenen_split, kategorien, kreislauf, kirchensteuer_baustein, deltas
...
def baue_kontext(b: Bescheid) -> dict:
    ...
    if m["kinder"]:
        ...
        gp = guenstigerpruefung(einkommen - fb, fb, kg if kg else D("0"), pv, jahr, splitting)
        kindervorteil = gp
    ...
    kat_ctx = [{
        ...
        "einheiten": [
            {"text": (f"{str(e['anzahl']).replace('.', ',')} × {e['einheit']}"
                      if e["anzahl"] is not None else e["einheit"]),
             "label": e["label"] + (f" · {e['basis_zusatz']}" if e.get("basis_zusatz") else "")}
            for e in k.einheiten],
    } for k in kats]
    delta_ctx = deltas(basis)

    kg_erhalten = _w(f.hinzurechnung_kindergeld) or D("0")
    beitrag_soziales = next((k.betrag for k in kats if k.id == "soziale_sicherung"), D("0"))
    zurueck = kg_erhalten + (pv if m["progressionsvorbehalt"] else D("0"))
    ...
    anrede = ("Sehr geehrte Familie" if splitting and m["kinder"]
              else "Sehr geehrtes Paar" if splitting
              else "Sehr geehrte Steuerzahlerin, sehr geehrter Steuerzahler")
    ...
    return {
        ...
        "deltas": delta_ctx,
        "kreislauf": kreis,
        "kg_erhalten": euro(kg_erhalten) if kg_erhalten else None,
        "lohnersatz": euro(pv) if pv else None,
    }
